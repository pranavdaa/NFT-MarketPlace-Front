<template>
  <div class="navbar-wrapper fixed-top">
    ...
    <div
      class="alert alert-dismissible text-center ms-0 fade show w-100"
      role="alert"
    >
      <div class="text-white pt-1 alert-text">
        <strong>Warning:</strong> Currently there is an issue with the
        third-party NFT balance API service that the marketplace depends on.
        Some users may see inaccuracies in "My Account" of the NFTs they own.
        Note that all ownership is securely stored on the blockchain, so don't
        worry about the inaccuracies - it is just a temporary issue. We are in
        touch with the service provider and it should be fixed soon.
      </div>
    </div>
  </div>
</template>
